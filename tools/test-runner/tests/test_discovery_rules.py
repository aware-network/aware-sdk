...
def _load_internal_manifest() -> TestSuiteDiscovery:
    repo_root = Path(__file__).resolve().parents[3]
    manifest_path = repo_root / "tools" / "test-runner" / "configs" / "manifests" / "internal" / "manifest.json"
    manifest = load_manifest(manifest_file=str(manifest_path))
    return TestSuiteDiscovery(str(repo_root), manifest)


def test_environments_category_discovery() -> None:
    discovery = _load_internal_manifest()

    categories = discovery.available_categories()
    assert "environments" in categories

    suites = discovery.get_suites_by_category("environments")
    assert "aware_environments" in suites
    assert suites["aware_environments"].path.endswith("environments/tests")
