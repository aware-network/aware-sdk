...
class SimpleChangeDetector:
    ...
    @staticmethod
    def detect_changes(previous_state: Dict[str, FileMetadata], current_state: Dict[str, FileMetadata]) -> Dict[ChangeType, list]:
        """Detect changes between two file system states."""
        current_files = set(current_state.keys())
        previous_files = set(previous_state.keys())
        
        changes = {
            ChangeType.ADDED: list(current_files - previous_files),
            ChangeType.DELETED: list(previous_files - current_files),
            ChangeType.MODIFIED: [
                file_path
...
class FileSystemWatcher:
    ...
    def __init__(
        ...
        # GitIgnore-style filtering (mirrors Dart implementation)
        self._ignored_extensions = {
            '.pyc', '.pyo', '.pyd', '.so', '.dll', '.dylib',
            '.exe', '.o', '.a', '.lib', '.jar', '.class',
    ...
    async def _emit_change_events(self, changes: Dict[ChangeType, list], current_state: Dict[str, FileMetadata]) -> None:
        """
        ...
        # Handle added files
        for file_path in changes.get(ChangeType.ADDED, []):
        ...
        for file_path in changes.get(ChangeType.MODIFIED, []):
            event = FileChangeEvent(
        ...
        for file_path in changes.get(ChangeType.DELETED, []):
            event = FileChangeEvent(
...
class RepositoryFileSystemWatcher:
    ...
    def __init__(self, repository, poll_interval: float = 5.0):
        """
        ...
        config = Config(
            file_system=FileSystemConfig(
                root_path=repository.workspace_root,
                generate_tree=False
            )
        )
        
        # Initialize base watcher
        self.watcher = FileSystemWatcher(config, poll_interval=poll_interval)
